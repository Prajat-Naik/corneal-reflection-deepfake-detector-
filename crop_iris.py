...
def segment_iris(face_crop, eye_mask,radius_min_para,radius_max_para):
    ...
    edges = canny(roi_gray.astype(np.float64), sigma=2.0, low_threshold=40, high_threshold=70)

    edges_mask = canny((roi_mask * 255).astype(np.float64), sigma=1.5, low_threshold=1, high_threshold=240)
